...
ab_auth_assert_authorized("ListActions");

function ab_render_actions(array $actions, int $page, int $page_size, int $total_page, string $url): void
{
...
$statement->execute();
$actions = ab_escape_array_of_arrays($statement->fetchAll(PDO::FETCH_ASSOC));

ab_template_render_header();
ab_template_render_sidebar();
ab_render_actions($actions, $page, $page_size, $total_page, "/actions");
